...
# CloudFrontのIPアドレス範囲を取得(プレフィックスリスト)
data "aws_ec2_managed_prefix_list" "cloudfront_prefix_list" {
  name = "com.amazonaws.global.cloudfront.origin-facing"
}

# CloudFrontが、httpでアクセスしてきた場合は、httpsにリダイレクトするため、httpは不要
# resource "aws_security_group_rule" "alb_sg_rule_inbound_http" {
#   type = "ingress"
#   from_port = 80
#   to_port = 80
#   protocol = "tcp"
#   # cidr_blocks = ["0.0.0.0/0"] # Route53=>ALBの場合は、これを使う。
#   prefix_list_ids = [data.aws_ec2_managed_prefix_list.cloudfront_prefix_list.id] # CloudFront=>ALBの場合は、これを使う。
